...
> like `92 pages · 15 episodes · 28 video scenes`. **If it says 1 episode, you are
> running an old copy.** The site footer agrees — it should read *Topic n of 92*.
> Delete stale folders rather than keeping them around.
...
```
content/module-3/3.8-transformers.md
```
...
```yaml
  - title: "Module 3 — Deep Learning for Architecture"
    episodes:
      - module-3/3.7-appliance-energy-demo.md
      - module-3/3.8-transformers.md          # ← your new line
```

The sidebar, the topic numbering, the "Topic 46 of 92" counter, the Previous/Next
buttons, the module and episode landing pages, and the instructor block all generate
themselves. You never edit them.

**The fastest start is to copy an existing episode.** `module-3/3.1-neuron.md` uses
nearly every feature; `module-3/3.2-how-networks-learn.md` is a simpler one.
...
```yaml
---
episode: "3.8"                       # ALWAYS quote this. See the warning below.
title: Transformers for sequences
duration: 25
...
Cell counts: `cards`, `compare`, `refs` and `workflow` take **3**; `stats` takes
**2**; `resources` takes **5** (`KIND | Title | Source | Description | URL`).

### Reflect questions

Mark the correct option with `*` and the card gains a **Check answer** button:

```markdown
:::reflect
If you doubled the depth, what would happen to the test score?
...
Diagrams are SVG written in Python, in `engine/figures.py`, so they stay sharp in both
the site and the video and can be diffed in git. Add a function, register it in the
...
```
scene_id = sha256(title + narration + every block's rendered frame)[:12]
```
...
python engine/build.py --video --engine edge    # render
```
...
python engine/build.py --video --engine edge  site + MP4
...
engine/                       the pipeline — nobody edits this
  ...
  figures.py                    static SVG diagrams
  widgets.py                    interactive figures
  render_web.py                 → site/
  render_video.py               → .cache/ → dist/course.mp4
  theme/site.css  site.js       black-and-white theme, Montserrat + IBM Plex Mono
...
Currently: **2.1, 2.2, 2.5** are starter drafts, and **2.6** needs the UW
building-energy CSV dropped into `assets/data/`.
